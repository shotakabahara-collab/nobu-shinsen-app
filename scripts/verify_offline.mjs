...
const required=['pyodide/pyodide.js','pyodide/pyodide.asm.js','pyodide/pyodide.asm.wasm','pyodide/pyodide-lock.json','pyodide/python_stdlib.zip','runtime-worker.js','runtime_bundle_b223.tgz','canonical_officer_catalog.json','manifest.webmanifest','apple-touch-icon.png','icon-192.png','icon-512.png'];
...
const catalog=JSON.parse(await readFile(join('dist','canonical_officer_catalog.json'),'utf8'));
...
if(catalog.canonicalVersion!==lock.canonicalVersion||catalog.canonicalArchiveSha256!==lock.archiveSha256)throw new Error('canonical officer catalog release lock mismatch');
if(!Array.isArray(catalog.officers)||catalog.officerCount!==catalog.officers.length)throw new Error('canonical officer catalog is malformed');
...
console.log(`offline precache verified: ${required.length} required assets; ${catalog.officerCount} canonical officers; runtime bundle ${bundleSha}`);
