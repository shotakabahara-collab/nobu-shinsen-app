...
test('searches the complete canonical officer and skill catalogs with staged runtime evaluation',async({page})=>{
 ...
 await expect(page.getByText(/全カタログ探索と上位2候補の各100戦評価が完了しました/)).toBeVisible({timeout:540_000});
 ...
 await expect(panel.getByText(/候補4件を各20戦で再比較し、上位2件を各100戦で確定評価/)).toBeVisible();
 await expect(panel.getByText(/100戦勝率/)).toHaveCount(2);
 await expect(panel.getByText(/勝・.*敗・.*引分（完了100戦）/).first()).toBeVisible();
 await expect(panel.getByText(/候補1/).first()).toBeVisible();await expect(panel.getByText(/候補2/).first()).toBeVisible();await expect(page.getByRole('button',{name:'この役割配置を30×3再評価'})).toBeVisible();
