...
import {buildBattleSnapshot,buildInitialActionOrderTrace,enrichExampleRoles,enrichTraceRoles,extractRepresentativeTrace,formationsForBattleResult,parseBattleExamples,parseBattleSnapshot,type BattleExample,type BattleOfficerSnapshot,type BattleSide,type BattleSnapshot,type StatValues,type TroopChange,type TroopPoint} from '../battleLog/battleLogView';
...
function StatusSide({side,snapshot}:{side:BattleSide;snapshot:BattleSnapshot}){const value=snapshot.sides[side];return <section className="space-y-2"><div className="flex items-center justify-between"><h5 className="font-bold">編成{side}：{value.formationName}</h5><span className="text-xs text-slate-400">{value.troopType} Lv{value.troopLevel}</span></div><div className="grid gap-2 md:grid-cols-3">{value.officers.map(officer=><OfficerCard key={`${side}-${officer.slot}`} officer={officer}/>)}</div></section>}

function TroopBoard({points,label}:{points:TroopPoint[];label:string}){
 return <div><p className="mb-1 text-[10px] font-bold text-slate-500">{label}</p><div className="grid grid-cols-2 gap-1 sm:grid-cols-3">{points.map(point=><div key={`${point.side}-${point.officer}`} className="flex items-center justify-between rounded bg-slate-900 px-2 py-1 text-xs"><span className="truncate"><strong>{point.side}</strong> {point.officer}</span><span>{troops(point.troops)}</span></div>)}</div></div>;
}
...
function ChangeRows({changes,empty=false}:{changes:TroopChange[];empty?:boolean}){
 ...
 return <ul className="space-y-1">{changes.map((change,index)=><li key={`${change.side}-${change.officer}-${index}`} className={`rounded px-2 py-1 text-xs ${change.kind==='reserve'?'bg-indigo-950 text-indigo-300':change.delta>0?'bg-emerald-950 text-emerald-300':'bg-red-950 text-red-300'}`}><strong>{change.side} {change.officer}</strong>：{change.kind==='reserve'?`備兵 ${change.delta>0?'+':''}${troops(change.delta)}`:`${troops(change.before)} → ${troops(change.after)}（${change.delta>0?'+':''}${troops(change.delta)}）`}<span className="ml-1 text-[10px] opacity-75">{change.source}</span></li>)}</ul>;
}
...
function ExampleView({example}:{example:BattleExample}){
 return <section className="space-y-3" aria-label={`${example.outcome==='win'?'勝ち':'負け'}戦闘例`}>
  ...
  <div className="space-y-2">{example.turns.map(turn=><details key={turn.turn} className="rounded-xl border border-slate-700 bg-slate-950 p-3" open={turn.turn===1}>
   ...
   <div className="mt-3 space-y-3"><TroopBoard points={turn.startTroops} label="ターン開始兵数"/>
    {turn.status==='battle_ended'?<p className="rounded-lg bg-slate-900 p-3 text-sm text-slate-400">この戦闘はT{example.endedTurn}で終了済みです。T{turn.turn}の行動はありません。</p>:<>
     {turn.turnStartChanges.length>0&&<div><p className="mb-1 text-xs font-bold text-sky-300">ターン開始処理</p><ChangeRows changes={turn.turnStartChanges}/></div>}
     <ol className="space-y-2">{turn.actions.map(action=><li key={`${turn.turn}-${action.rank}-${action.side}-${action.officer}`} className="rounded-xl bg-slate-900 p-3">
...
export function BattleLogDetail({log,formations,onClose}:Props){
 const payload=log.payload;const stored=useMemo(()=>parseBattleSnapshot(payload),[payload]);const exampleSet=useMemo(()=>parseBattleExamples(payload),[payload]);
 const [snapshot,setSnapshot]=useState<BattleSnapshot|undefined>(stored);const [fallback,setFallback]=useState(false);const [statusError,setStatusError]=useState('');const [direction,setDirection]=useState<'forward'|'reverse'>('forward');const [selectedExample,setSelectedExample]=useState(0);
 useEffect(()=>{
 ...
 return <section aria-label="Battle Log詳細" className="space-y-4 rounded-2xl border border-amber-700 bg-slate-950 p-4">
  ...
  <section aria-label="100戦勝率" className="rounded-xl border border-amber-700 bg-slate-900 p-4"><div className="flex items-center gap-2"><Swords className="size-5 text-amber-300"/><h5 className="font-bold">100戦勝率</h5></div><p className="mt-2 text-4xl font-black text-amber-400">{(log.winRate*100).toFixed(1)}%</p>{exampleSet?<><p className="mt-1 text-xs text-slate-400">100戦／方向 × 正逆2方向・完了{exampleSet.completedTrials}試行</p><div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded bg-emerald-950 p-2 text-emerald-300">勝ち<br/><strong className="text-lg">{exampleSet.candidateWins}</strong></div><div className="rounded bg-red-950 p-2 text-red-300">負け<br/><strong className="text-lg">{exampleSet.candidateLosses}</strong></div><div className="rounded bg-slate-800 p-2 text-slate-300">引分<br/><strong className="text-lg">{exampleSet.draws}</strong></div></div></>:<p className="mt-2 text-xs text-amber-300">旧ログのため100戦内訳は保存されていません。</p>}</section>
