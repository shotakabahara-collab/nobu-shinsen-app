...
export function BattleLogDetail({log,formations,onClose}:Props){
 ...
 const operational=(payload as Record<string,unknown>).formal_status==='OPERATIONAL_ONLY_RUNTIME_EVIDENCE_INCOMPLETE';
 return <section aria-label="Battle Log詳細" className="space-y-4 rounded-2xl border border-amber-700 bg-slate-950 p-4">
  ...
  <section aria-label="100戦結果" className="rounded-xl border border-amber-800 bg-slate-900 p-3"><p className="text-xs text-slate-400">{evaluation.summary.completedBattles}戦の勝率</p><p className="text-3xl font-black text-amber-300">{(evaluation.summary.winRate*100).toFixed(1)}%</p><div className="mt-2 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-lg bg-emerald-950 p-2 text-emerald-300">勝ち<br/><strong className="text-lg">{evaluation.summary.wins}</strong></div><div className="rounded-lg bg-red-950 p-2 text-red-300">負け<br/><strong className="text-lg">{evaluation.summary.losses}</strong></div><div className="rounded-lg bg-slate-800 p-2 text-slate-300">引分<br/><strong className="text-lg">{evaluation.summary.draws}</strong></div></div>{evaluation.summary.completedBattles!==evaluation.summary.requestedBattles&&<p className="mt-2 text-xs text-amber-300">要求{evaluation.summary.requestedBattles}戦のうち、runtime完了は{evaluation.summary.completedBattles}戦です。</p>}{operational&&<p role="alert" className="mt-2 rounded-lg bg-amber-950 p-2 text-xs leading-5 text-amber-200">効果本文が未収録の特性・戦法を含むため、この勝率は運用計算です。正式再評価には使用しません。</p>}</section>
