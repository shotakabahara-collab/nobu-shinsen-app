...
function detailedLog():BattleResult{
 ...
 const payload={battle_snapshot:snapshot,battle_examples:{schemaVersion:1,trialsPerDirection:50,directions:2,completedTrials:100,candidateWins:60,candidateLosses:40,draws:0,selectionPolicy:'test',examples:[example('win',100),example('loss',200)]}};
 return {...oldLog,id:'00000000-0000-4000-8000-000000000004',trials:50,payload};
}
...
describe('BattleLogDetail',()=>{
 ...
 it('shows 100 total battles, one win and one loss example, T1-T8 actions and troop changes',()=>{
  render(<BattleLogDetail log={detailedLog()} formations={[]} onClose={()=>{}}/>);
  const rate=screen.getByRole('region',{name:'100戦勝率'});expect(rate).toHaveTextContent('60.0%');expect(rate).toHaveTextContent('合計100戦（正逆50戦ずつ）');expect(rate).toHaveTextContent('60');expect(rate).toHaveTextContent('40');
