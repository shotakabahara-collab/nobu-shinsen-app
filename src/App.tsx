...
export default function App(){
 ...
 async function optimize(){
  ...
  try{
   ...
   for(let index=0;index<preliminary.length;index++){
    ...
    try{
     const measured=await runtimeClient.calculateSummary({candidate:candidateToRuntimeFormation(item.candidate,officerCatalog.officers,skillCatalog.skills,officerCatalog.unitLevelRule),target:optimizationTarget.id,target_spec:toRuntimeFormation(optimizationTarget),trials:50,blocks:1,seed:1326250000,include_detail:false},progress=>{
      setNotice(`候補${index+1}/${preliminary.length}を100戦で検証中… ${progress.completedBattles}/100`);
     });
   ...
   setNotice(completed?`${fullCatalog?'全カタログ探索':'所有範囲探索'}と上位${completed}候補の各100戦評価が完了しました`:'候補の100戦評価が成立しませんでした。未成立値は順位から除外しました');
  }
...
function RecommendationPanel({recommendations,selectedIndex,disabled,onSelect,targetId,scope}:{recommendations:RankedRecommendation[];selectedIndex:number;disabled:boolean;onSelect:(index:number)=>void;targetId:string;scope:ReturnType<typeof getSearchScope>}){
 ...
 return <div className="space-y-3" aria-label="最適編成候補">
  ...
  <p className="rounded-lg bg-slate-950 p-3 text-xs text-slate-400">評価 {scope.generated??0}/{scope.budget??0}・{scope.budget_cut?'予算打切り':'設定範囲完了'}。{typeof scope.role_placements_simulated==='number'&&`役割配置${scope.role_placements_simulated}件（${scope.role_families_shortlisted??0}編成組）を事前比較。`}{typeof scope.final_evaluation_completed==='number'&&`上位${scope.final_evaluation_completed}候補は各100戦を完了。`}表示順位は段階探索で評価した範囲内であり、大域的な絶対最適を保証しません。</p>
