...
export default function App(){
 ...
 useEffect(()=>{setSearchResult(null);setFormalResult(null);setSelectedRecommendationIndex(0);setRoleOverride(null);},[optimizationTargetId,optimizationTarget?.updatedAt]);
 ...
 async function optimize(){
  ...
  setRunning(true);setSearchResult(null);setFormalResult(null);setSelectedRecommendationIndex(0);setRoleOverride(null);setNotice(`${optimizationTarget.name}への最適候補を探索中…`);
...
function ResultCard({result,label}:{result:RuntimeResult;label:string}){return <div className="rounded-xl bg-slate-950 p-4"><p className="text-sm text-slate-400">{label}</p><p className="text-3xl font-black text-amber-400">{typeof result.win_rate==='number'?`${(result.win_rate*100).toFixed(1)}%`:'—'}</p><p className="text-sm text-slate-400">HP差 {typeof result.hp_diff==='number'?result.hp_diff.toFixed(1):'—'}・{typeof result.trials_total==='number'?`${result.trials_total}戦`:'100戦'}</p><p className="mt-2 text-xs text-slate-500">{ENGINE_RESULT_LABEL}</p></div>}
