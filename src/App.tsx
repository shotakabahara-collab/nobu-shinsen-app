...
const MATCHUP_TRIALS_PER_DIRECTION=50;
...
export default function App(){
 ...
 async function calculate(){
  ...
  setRunning(true);setNotice(`${ENGINE_DISPLAY_NAME}で100戦（正逆50戦ずつ）の勝率と勝敗例を計算中…`);
  try{
   const raw=await runtimeClient.calculate({candidate:toRuntimeFormation(formationA),target:formationB.id,target_spec:toRuntimeFormation(formationB),trials:MATCHUP_TRIALS_PER_DIRECTION,blocks:1,seed:1326230000,include_detail:true,include_examples:true});
   ...
   setResult(value);
   await store.saveBattleResult({id:crypto.randomUUID(),allyId:formationA.id,enemyId:formationB.id,createdAt:new Date().toISOString(),status:'completed',winRate:value.win_rate!,hpDiff:typeof value.hp_diff==='number'?value.hp_diff:null,trials:MATCHUP_TRIALS_PER_DIRECTION,blocks:1,runtime:value.runtime,payload:value});
...
function ResultCard({result,label}:{result:RuntimeResult;label:string}){const trials=typeof result.trials_per_direction==='number'?result.trials_per_direction:MATCHUP_TRIALS_PER_DIRECTION;return <div className="rounded-xl bg-slate-950 p-4"><p className="text-sm text-slate-400">{label}</p><p className="text-3xl font-black text-amber-400">{typeof result.win_rate==='number'?`${(result.win_rate*100).toFixed(1)}%`:'—'}</p><p className="text-sm text-slate-400">合計 {trials*2}戦（正逆{trials}戦ずつ）・HP差 {typeof result.hp_diff==='number'?result.hp_diff.toFixed(1):'—'}</p><p className="mt-2 text-xs text-slate-500">{ENGINE_RESULT_LABEL}</p></div>}
