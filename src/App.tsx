...
import {attachBattleSnapshot,buildBattleSnapshot,parseBattleSummary} from './battleLog/battleLogView';
...
const BATTLE_TRIALS_PER_DIRECTION=50;
const BATTLE_TOTAL_TRIALS=BATTLE_TRIALS_PER_DIRECTION*2;
...
export default function App(){
 ...
 async function calculate(){
  ...
  try{
   const raw=await runtimeClient.calculate({candidate:toRuntimeFormation(formationA),target:formationB.id,target_spec:toRuntimeFormation(formationB),trials:BATTLE_TRIALS_PER_DIRECTION,blocks:1,seed:1326230000,include_detail:true});
   ...
   setResult(value);
   const summary=parseBattleSummary(value);
   await store.saveBattleResult({id:crypto.randomUUID(),allyId:formationA.id,enemyId:formationB.id,createdAt:new Date().toISOString(),status:'completed',winRate:value.win_rate!,hpDiff:typeof value.hp_diff==='number'?value.hp_diff:null,trials:summary?.requestedBattles??BATTLE_TOTAL_TRIALS,blocks:1,runtime:value.runtime,payload:value});
 ...
 return <Shell>
  ...
  <main className="space-y-4 pb-24">
   ...
   {page==='battle'&&<>
    ...
    <section className="space-y-3 rounded-2xl border border-amber-800 bg-slate-900 p-4"><h3 className="font-bold">登録編成同士を対戦</h3><p className="text-xs text-slate-400">順方向50戦＋逆方向50戦の計100戦で勝率を算出します。</p><label className="block text-sm text-slate-400">編成A<select aria-label="編成A" className="mt-1 w-full rounded-lg bg-slate-950 p-3" value={formationAId} onChange={event=>setFormationAId(event.target.value)}><option value="">選択</option>{store.formations.map(formation=><option key={formation.id} value={formation.id} disabled={formation.id===formationBId}>{formation.name}</option>)}</select></label><label className="block text-sm text-slate-400">編成B<select aria-label="編成B" className="mt-1 w-full rounded-lg bg-slate-950 p-3" value={formationBId} onChange={event=>setFormationBId(event.target.value)}><option value="">選択</option>{store.formations.map(formation=><option key={formation.id} value={formation.id} disabled={formation.id===formationAId}>{formation.name}</option>)}</select></label>{running?<Button variant="danger" className="w-full" onClick={()=>cancelRuntime('計算を中止しました')}><Square className="mr-2 size-4"/>中止</Button>:<Button className="w-full" disabled={!formationA||!formationB||formationA.id===formationB.id} onClick={()=>void calculate()}><Swords className="mr-2 size-4"/>100戦で対戦</Button>}{result&&<ResultCard result={result} label={`${formationA?.name??'編成A'}の100戦勝率`}/>}</section>
...
function ResultCard({result,label}:{result:RuntimeResult;label:string}){const summary=parseBattleSummary(result);return <div className="rounded-xl bg-slate-950 p-4"><p className="text-sm text-slate-400">{label}</p><p className="text-3xl font-black text-amber-400">{typeof result.win_rate==='number'?`${(result.win_rate*100).toFixed(1)}%`:'—'}</p>{summary&&<p className="text-sm text-slate-300">勝 {summary.wins}・敗 {summary.losses}・引分 {summary.draws}（完走 {summary.completedBattles}/{summary.requestedBattles}戦）</p>}<p className="text-sm text-slate-400">HP差 {typeof result.hp_diff==='number'?result.hp_diff.toFixed(1):'—'}</p><p className="mt-2 text-xs text-slate-500">{ENGINE_RESULT_LABEL}</p></div>}
