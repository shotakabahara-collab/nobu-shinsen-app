...
import {applyFinalRecommendationEvaluations,buildRecommendationReasons,candidateSkillLines,candidateToRuntimeFormation,getRankedRecommendations,getSearchScope,rankCompletedRecommendationEvaluations,recommendationForRoleOrder,recommendationToFormation,type FinalRecommendationEvaluation,type RankedRecommendation} from './domain/recommendation';
...
export default function App(){
 ...
 async function optimize(){
  ...
  try{
   const screened=await runtimeClient.search(buildTargetOptimizationRequest(optimizationTarget,store.formations,store.warriors,store.skills,optimizationCatalogScope));
   const preliminary=getRankedRecommendations(screened);
   if(!preliminary.length){setSearchResult(screened);setNotice('実戦評価を完了した候補がありません。未成立の0.0%・HP差0.0は表示していません');return;}
   const [officerCatalog,skillCatalog]=await Promise.all([loadCanonicalOfficerCatalog(),loadCanonicalSkillCatalog()]);
   const tournamentCandidates=preliminary.slice(0,4),tournamentEvaluations:FinalRecommendationEvaluation[]=[];
   for(let index=0;index<tournamentCandidates.length;index++){
    const item=tournamentCandidates[index]!;
    try{
     const measured=await runtimeClient.calculateSummary({candidate:candidateToRuntimeFormation(item.candidate,officerCatalog.officers,skillCatalog.skills,officerCatalog.unitLevelRule),target:optimizationTarget.id,target_spec:toRuntimeFormation(optimizationTarget),trials:10,blocks:1,seed:1326240000,include_detail:false},progress=>{
      setNotice(`事前候補${index+1}/${tournamentCandidates.length}を20戦で再比較中… ${progress.completedBattles}/20`);
     },20);
     tournamentEvaluations.push({candidate:item.candidate,result:measured});
    }catch(error){if(error instanceof RuntimeCancelledError)throw error;}
   }
   const rankedTournament=rankCompletedRecommendationEvaluations(tournamentEvaluations,20);
   const finalists=rankedTournament.slice(0,2);
   const evaluations:FinalRecommendationEvaluation[]=[];
   for(let index=0;index<finalists.length;index++){
    const item=finalists[index]!;
    try{
     const measured=await runtimeClient.calculateSummary({candidate:candidateToRuntimeFormation(item.candidate,officerCatalog.officers,skillCatalog.skills,officerCatalog.unitLevelRule),target:optimizationTarget.id,target_spec:toRuntimeFormation(optimizationTarget),trials:50,blocks:1,seed:1326250000,include_detail:false},progress=>{
      setNotice(`決勝候補${index+1}/${finalists.length}を100戦で検証中… ${progress.completedBattles}/100`);
     });
     evaluations.push({candidate:item.candidate,result:measured});
    }catch(error){if(error instanceof RuntimeCancelledError)throw error;}
   }
   const value=applyFinalRecommendationEvaluations(screened,optimizationTarget.id,evaluations,{requested:tournamentCandidates.length,completed:rankedTournament.length,failed:tournamentCandidates.length-rankedTournament.length,battlesPerCandidate:20,finalistLimit:2,finalRequested:finalists.length});
   setSearchResult(value);
   const completed=getRankedRecommendations(value).length;
   setNotice(completed?`${fullCatalog?'全カタログ探索':'所有範囲探索'}と上位${completed}候補の各100戦評価が完了しました`:'候補の段階実戦評価が成立しませんでした。未成立値は順位から除外しました');
  }
...
function RecommendationPanel({recommendations,selectedIndex,disabled,onSelect,targetId,scope}:{recommendations:RankedRecommendation[];selectedIndex:number;disabled:boolean;onSelect:(index:number)=>void;targetId:string;scope:ReturnType<typeof getSearchScope>}){
 ...
 if(!recommendations.length){const targetStopped=(scope.target_formal_stops?.length??0)>0;return <p role="alert" className="rounded-xl bg-red-950 p-3 text-sm leading-6 text-red-300">{targetStopped?'対象編成が正本runtimeの戦闘開始条件を満たさないため、勝率を計測できませんでした。編成の兵種・戦法構成を確認してください。':'完了した実戦データを持つ推奨候補を生成できませんでした。未成立の0.0%・HP差0.0は結果から除外しています。'}{!targetStopped&&!fullCatalog&&' 登録武将・戦法・編成内容を確認してください。'}</p>;}
 ...
 return <div className="space-y-3" aria-label="最適編成候補">
  ...
  <p className="rounded-lg bg-slate-950 p-3 text-xs text-slate-400">評価 {scope.generated??0}/{scope.budget??0}・{scope.budget_cut?'予算打切り':'設定範囲完了'}。{typeof scope.role_placements_simulated==='number'&&`役割配置${scope.role_placements_simulated}件（${scope.role_families_shortlisted??0}編成組）を事前比較。`}{typeof scope.tournament_screen_completed==='number'?`候補${scope.tournament_screen_completed}件を各${scope.tournament_screen_battles_per_candidate??20}戦で再比較し、上位${scope.final_evaluation_completed??0}件を各100戦で確定評価。`:typeof scope.final_evaluation_completed==='number'?`事前比較1位は100戦を完了。`:''}表示順位は段階探索で評価した範囲内であり、大域的な絶対最適を保証しません。</p>
  {recommendations.map((item,index)=>{const selected=index===selectedIndex;const rate=item.win_rates?.[targetId]??item.avg_win_rate??item.min_win_rate;const hp=item.hp_diffs?.[targetId];const evidence=item.battle_evidence?.[targetId];const final100=evidence?.measurement_stage==='FINAL'&&evidence.completed_battles===100;return <button type="button" disabled={disabled} key={`${item.candidate.officers.join('-')}-${index}`} aria-pressed={selected} onClick={()=>onSelect(index)} className={`w-full rounded-xl border p-4 text-left disabled:opacity-60 ${selected?'border-emerald-400 bg-emerald-950/40':'border-slate-700 bg-slate-950'}`}><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-emerald-300">候補{index+1} {selected&&<span className="ml-1 text-xs">選択中</span>}</p><p className="mt-1">大将 {item.candidate.officers[0]}／{item.candidate.unit}</p><p className="mt-1 text-xs text-slate-400">副将1 {item.candidate.officers[1]}・副将2 {item.candidate.officers[2]}</p></div>{selected&&<CheckCircle2 className="size-5 shrink-0 text-emerald-400"/>}</div><p className="mt-2 text-sm font-bold text-slate-200">{final100?'100戦勝率':'事前選別勝率'} {typeof rate==='number'?`${(rate*100).toFixed(1)}%`:'—'}・平均HP差 {typeof hp==='number'?`${hp>=0?'+':''}${hp.toFixed(1)}`:'—'}</p>{final100&&<p className="mt-1 text-xs text-slate-400">{evidence?.wins??0}勝・{evidence?.losses??0}敗・{evidence?.draws??0}引分（完了100戦）</p>}<div className="mt-3 space-y-1 text-xs text-slate-400">{candidateSkillLines(item.candidate).map(line=><p key={line}>{line}</p>)}</div><div className="mt-3 border-t border-slate-700 pt-3"><p className="text-xs font-bold text-amber-300">推奨理由</p><ul className="mt-1 space-y-1 text-xs text-slate-400">{buildRecommendationReasons(item,index,targetId,scope).map(reason=><li key={reason}>・{reason}</li>)}</ul></div></button>;})}
 </div>;
}
