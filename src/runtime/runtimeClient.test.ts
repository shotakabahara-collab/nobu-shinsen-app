...
describe('RuntimeClient',()=>{
 ...
 it('rejects every pending operation with a Japanese public error when the worker fails',async()=>{vi.stubGlobal('Worker',WorkerMock);const client=new RuntimeClient(),result=client.calculate(request),worker=WorkerMock.instances[0]!;worker.onerror?.(new ErrorEvent('error'));await expect(result).rejects.toThrow('計算を完了できませんでした（UNKNOWN-001）');expect(worker.terminate).toHaveBeenCalled();});
 ...
 it('hides raw postMessage failures behind a Japanese public error',async()=>{vi.stubGlobal('Worker',WorkerMock);const client=new RuntimeClient();const worker=new WorkerMock();worker.postMessage.mockImplementation(()=>{throw new Error('clone failed');});(client as unknown as {worker:Worker}).worker=worker as unknown as Worker;await expect(client.calculate(request)).rejects.toThrow('計算を完了できませんでした（UNKNOWN-001）');});
});
