import {z} from 'zod';
const runtimeFormationSchema=z.object({officers:z.tuple([z.string(),z.string(),z.string()]),awaken:z.tuple([z.number().int().min(0).max(5),z.number().int().min(0).max(5),z.number().int().min(0).max(5)]),unit:z.enum(['騎馬','足軽','弓','鉄砲']),unit_level:z.number().int().min(1).max(10),troops:z.literal(10000),skills:z.tuple([z.string(),z.string(),z.string(),z.string(),z.string(),z.string()]),stats:z.array(z.record(z.string(),z.number())).optional(),fixed_placement:z.boolean().default(true),ignore_formal_overlap:z.boolean().default(true)});
