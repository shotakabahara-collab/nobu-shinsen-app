...
describe('buildTargetOptimizationRequest',()=>{
 it('defaults to the full canonical catalog while preserving registered seeds and owned awaken overrides',()=>{
  ...
  expect(request.known_awaken_overrides).toEqual([{name:'追加武将',awaken:5}]);
  expect(request.skill_pool).toEqual([{name:'装着X'}]);
  expect(request.catalog_scope).toBe('canonical_all');
  expect(request.units).toEqual(['足軽','騎馬','鉄砲','弓']);
  expect(request.structural_budget).toBe(4800);
  expect(request.role_family_shortlist).toBe(4);
  expect(request.trials).toBe(2);
 });
 ...
 it('keeps the lower-cost owned-only search as an explicit option',()=>{
  const target=formation('対象');
  const request=buildTargetOptimizationRequest(target,[target],[],[],'owned_only');
  expect(request.catalog_scope).toBe('owned_only');
  expect(request.structural_budget).toBe(1200);
