...
function isRecoverableWasmCrash(error:unknown):boolean{
 ...
 return (normalized.includes('new_error@')||normalized.includes('wasm-function'))&&!normalized.includes('pythonerror')&&!normalized.includes('traceback');
}
...
export class RuntimeClient{
 ...
 private ensureWorker(){
  ...
  worker.onerror=event=>this.failAllRaw(new Error(event.message||'計算エンジンを起動できませんでした'),worker);
  worker.onmessageerror=()=>this.failAllRaw(new Error('計算エンジンの応答を読み取れませんでした'),worker);
  this.worker=worker;return worker;
 }

 private recycleWorker(){this.disposeWorker(this.worker);}
 private async yieldAfterRecycle(){await new Promise<void>(resolve=>setTimeout(resolve,100));}
 ...
 private async runBatchOnce(request:CalculateRequest,trials:number,forwardSeed:number,reverseSeed:number,generation:number):Promise<CalculateBatchResult>{
  ...
  }finally{this.recycleWorker();await this.yieldAfterRecycle();}
 }
 ...
 private async runDetailRaw(request:CalculateRequest,sample:Sample,generation:number):Promise<RuntimeResult>{
  this.assertActive(generation);
  try{return await this.runRaw('detail',{candidate:request.candidate,target:request.target,target_spec:request.target_spec,seed:sample.seed,direction:sample.direction});}
  finally{this.recycleWorker();await this.yieldAfterRecycle();}
 }
 ...
 private async detailForOutcome(request:CalculateRequest,outcome:Exclude<Outcome,'draw'>,samples:Sample[],generation:number):Promise<{schemaVersion:1;direction:Sample['direction'];seed:number;outcome:Exclude<Outcome,'draw'>;detail:RuntimeResult}>{
  ...
  for(const sample of candidates){
   for(let attempt=0;attempt<2;attempt++){
    ...
    catch(error){if(error instanceof RuntimeCancelledError)throw error;lastError=error;if(!isRecoverableWasmCrash(error))break;}
   }
  }
 ...
 private async calculateBatched(requestValue:CalculateRequest,onProgress?:((progress:BattleCalculationProgress)=>void)):Promise<RuntimeResult>{
  ...
  while(completedPerDirection<TOTAL_PER_DIRECTION){
   ...
   const batches=await this.runChunk(request,trials,forwardSeed,reverseSeed,generation);
  ...
  for(const outcome of outcomes){
   examples.push(await this.detailForOutcome(request,outcome,samples,generation));
  ...
  return {
   type:'simulation',version:'adapter-v1-browser-100-isolated-batches',runtime:firstBatch?.runtime??'B223_CANONICAL_PYTHON_VIA_PYODIDE',target:request.target,
