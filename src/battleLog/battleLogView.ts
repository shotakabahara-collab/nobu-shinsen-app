...
export type BattleExampleTurn={turn:number;status:'active'|'battle_ended';startTroops:TroopPoint[];endTroops:TroopPoint[];turnStartEvents:string[];turnStartChanges:TroopChange[];actions:BattleExampleAction[];turnEndChanges:TroopChange[]};
...
export function buildBattleSnapshot(formationA:Formation,formationB:Formation,catalog:CanonicalOfficerStatsCatalog):BattleSnapshot{return {schemaVersion:1,source:'canonical_officer_stats_catalog',sides:{A:sideSnapshot('A',formationA,catalog),B:sideSnapshot('B',formationB,catalog)}};}
...
export function parseBattleSnapshot(payload:Record<string,unknown>):BattleSnapshot|undefined{const candidate=record(payload.battle_snapshot);if(!candidate||candidate.schemaVersion!==1||candidate.source!=='canonical_officer_stats_catalog')return undefined;const sides=record(candidate.sides),a=record(sides?.A),b=record(sides?.B);if(!a||!b||array(a.officers).length!==3||array(b.officers).length!==3)return undefined;return candidate as unknown as BattleSnapshot;}
export function parseBattleExamples(payload:Record<string,unknown>):BattleExampleSet|undefined{const candidate=record(payload.battle_examples);if(!candidate||candidate.schemaVersion!==1||candidate.directions!==2)return undefined;const examples=array(candidate.examples);if(!examples.every(value=>record(value)&&Array.isArray(record(value)?.turns)))return undefined;return candidate as unknown as BattleExampleSet;}
...
export function extractRepresentativeTrace(payload:Record<string,unknown>,direction:'forward'|'reverse'):RepresentativeTrace|undefined{
 ...
 const turns:ActionOrderTurn[]=Object.entries(digestTurns).map(([turnValue,rows])=>({turn:Number(turnValue),entries:array(rows).map((rowValue):ActionOrderEntry=>{const row=record(rowValue)??{};const raw:'A'|'B'=row.side==='B'?'B':'A';const side=canonicalSide(direction,raw);return {rank:numberOrNull(row.rank)??0,side,rawSide:raw,officer:stringOr(row.officer,'未確認'),role:'',effectiveSpeed:numberOrNull(row.effective_speed),baseSpeed:numberOrNull(row.base_speed),timedSpeedBonus:numberOrNull(row.timed_speed_bonus)??0,persistentSpeedBonus:numberOrNull(row.persistent_speed_bonus)??0};}).sort((a,b)=>a.rank-b.rank)})).filter(row=>Number.isFinite(row.turn)&&row.entries.length>0).sort((a,b)=>a.turn-b.turn);
...
export function buildInitialActionOrderTrace(snapshot:BattleSnapshot,direction:'forward'|'reverse'):RepresentativeTrace{
 const entries=[...snapshot.sides.A.officers,...snapshot.sides.B.officers].sort((a,b)=>{const speedA=a.actionOrderSpeed??Number.NEGATIVE_INFINITY,speedB=b.actionOrderSpeed??Number.NEGATIVE_INFINITY;return speedB-speedA||a.side.localeCompare(b.side)||a.slot-b.slot;}).map((officer,index):ActionOrderEntry=>({rank:index+1,side:officer.side,rawSide:rawSide(direction,officer.side),officer:officer.name,role:officer.role,effectiveSpeed:officer.actionOrderSpeed,baseSpeed:officer.allocated.speed,timedSpeedBonus:0,persistentSpeedBonus:0}));
...
export function enrichTraceRoles(trace:RepresentativeTrace,snapshot:BattleSnapshot|undefined):RepresentativeTrace{if(!snapshot)return trace;return {...trace,turns:trace.turns.map(turn=>({...turn,entries:turn.entries.map(entry=>{const officer=snapshot.sides[entry.side].officers.find(value=>value.name===entry.officer);return {...entry,role:officer?.role??entry.role};})}))};}
export function enrichExampleRoles(example:BattleExample,snapshot:BattleSnapshot|undefined):BattleExample{if(!snapshot)return example;return {...example,turns:example.turns.map(turn=>({...turn,turnStartEvents:turn.turnStartEvents??[],actions:turn.actions.map(action=>{const officer=snapshot.sides[action.side].officers.find(value=>value.name===action.officer);return {...action,role:officer?.role??action.role};})}))};}
