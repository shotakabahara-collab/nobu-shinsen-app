...
export function classifyRuntimeError(error:unknown):RuntimeErrorInfo{
 ...
 if(has('memoryerror','out of memory','allocation failed'))return {code:'RUNTIME-002',title:'計算量が端末の上限を超えました',message:'端末のメモリ不足により対戦計算を完了できませんでした。',guidance:['他のアプリを終了してから再試行してください。','Safariまたはホーム画面アプリを完全終了してから100戦を再実行してください。'],detail,retryable:true};
