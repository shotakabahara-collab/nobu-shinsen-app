...
export function classifyRuntimeError(error:unknown):RuntimeErrorInfo{
 ...
 if(has('memoryerror','out of memory','allocation failed')||((has('new_error@','wasm-function')&&has('pyodide.asm.js'))&&!has('pythonerror','traceback')))return {code:'RUNTIME-002',title:'端末上で対戦エンジンが停止しました',message:'iPhoneのWebAssembly実行領域が不足し、対戦計算を継続できませんでした。アプリは100戦を分割してメモリを解放する方式に対応しています。',guidance:['アプリを完全に終了し、最新版を読み込んでから再試行してください。','改善しない場合は、他のアプリを終了してSafariから公式URLを再読み込みしてください。'],detail,retryable:true};
 if(has('pyodide','wasm-function','pythonerror','new_error','traceback'))return {code:'RUNTIME-001',title:'対戦エンジンでエラーが発生しました',message:`${ENGINE_DISPLAY_NAME}が計算を完了できませんでした。編成データまたはエンジン内部処理に原因がある可能性があります。`,guidance:['編成内容を確認して、もう一度計算してください。','同じエラーが続く場合は「詳細をコピー」して報告してください。'],detail,retryable:true};
