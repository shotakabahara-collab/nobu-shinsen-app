...
export const formationSchema=z.object({id:z.string().uuid(),name:z.string().trim().min(1),kind:z.enum(['ally','enemy']),troopType:z.enum(troopTypes),troopLevel:z.number().int().min(1).max(11),troops:z.literal(10000,{error:'b223正式評価の兵力は武将ごとに10,000固定です'}),warriors:z.tuple([warriorSchema,warriorSchema,warriorSchema]),createdAt:z.string().datetime(),updatedAt:z.string().datetime()}).superRefine((v,ctx)=>{
