...
const officers:CanonicalOfficer[]=[
 {id:'A',name:'松永久秀',inherentSkill:'梟雄の計',unitLevelTraits:[{name:'砲術Ⅲ',unlockedAt:3,unitTypes:['鉄砲'],levelBonus:3,capUnlock:false,capBonus:0}]},
 {id:'B',name:'黒田官兵衛',inherentSkill:'七十二の計',unitLevelTraits:[]},
];
const valid={schemaVersion:1 as const,canonicalVersion:'v1326p15e2b223',canonicalArchiveSha256:'f'.repeat(64),unitLevelRule:{baseLevel:5 as const,defaultCap:10 as const,capUnlockMode:'unbounded' as const},officerCount:2,officers};
...
describe('canonicalOfficerCatalog',()=>{
 it('loads and validates a canonical catalog with unit level traits',async()=>{
  ...
  expect(result.officers[0]?.unitLevelTraits[0]).toMatchObject({name:'砲術Ⅲ',unlockedAt:3,levelBonus:3,capUnlock:false});
  expect(result.unitLevelRule).toEqual({baseLevel:5,defaultCap:10,capUnlockMode:'unbounded'});
 });
