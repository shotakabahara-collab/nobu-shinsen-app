...
- Battle Log保存・詳細表示、全146武将×全236戦法または所有範囲の段階探索、表示候補ごとの完了100戦評価、30×3正式再評価
...
全カタログ探索は34,456武将×戦法関係をDBメタデータで事前評価し、合法候補を正本runtimeで絞ります。役割配置の事前比較後、表示する上位候補は順方向50戦＋逆方向50戦を分割実行し、完了100戦の勝率・平均HP差・勝敗内訳で再順位付けします。戦闘が成立しなかった旧runtimeの `0.0% / HP差0.0` sentinelは結果として採用しません。組合せの直積総当たりではないため、大域的な絶対最適とは表示しません。
