...
- Battle Log保存・詳細表示、全146武将×全236戦法または所有範囲の段階探索、30×3正式再評価
...
全カタログ探索は34,456武将×戦法関係をDBメタデータで事前評価し、合法候補を正本runtimeで絞り、上位編成だけを共通seedで実戦比較します。組合せの直積総当たりではないため、大域的な絶対最適とは表示しません。
