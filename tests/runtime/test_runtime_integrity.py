...
sys.path.insert(0,str(ROOT/'scripts'));from canonical_archive import canonical_archive_bytes
...
class RuntimeIntegrityTest(unittest.TestCase):
 def test_canonical_and_battle_runtime_hashes(self):
  archive=canonical_archive_bytes(ROOT,LOCK['archive'],LOCK['archiveSha256']);self.assertEqual(sha_bytes(archive),LOCK['archiveSha256'])
  with zipfile.ZipFile(io.BytesIO(archive)) as z:self.assertEqual(sha_bytes(z.read(LOCK['battleRuntimePath'])),LOCK['battleRuntimeSha256'])
