...
'''
 ...
 def test_browser_worker_runs_small_isolated_balanced_batch_and_detail(self):
  with tempfile.TemporaryDirectory() as td:
   with tarfile.open(ROOT/'public/runtime_bundle_b223.tgz','r:gz') as tf:tf.extractall(td,filter='data')
   code=r'''
import json,sys
...
namespace={}
exec("import copy,gc,json,os,sys,tarfile,time\nsys.path.insert(0,'.')\n"+functions,namespace)
...
print(json.dumps({'batch':batch,'detail':detail},ensure_ascii=False))
'''
   ...
   payload=json.loads(result.stdout);batch=payload['batch'];detail=payload['detail']
   self.assertEqual(batch['type'],'simulation_batch');self.assertEqual(batch['trials_per_direction'],1)
