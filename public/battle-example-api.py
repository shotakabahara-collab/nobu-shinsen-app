...
def _parse_change(line, direction):
    ...
    if m:
        return {'turn':int(m.group(1)),'side':_canonical_side(direction,m.group(2)),'officer':m.group(3),'delta':-float(m.group(5)),'kind':'troops','source':m.group(4),'raw':line}
    m=re.match(r'^T(\d+) ([AB]):([^\s]+) 負傷兵回復 source=(.*?) heal=([0-9.]+)',line)
    if m:
        return {'turn':int(m.group(1)),'side':_canonical_side(direction,m.group(2)),'officer':m.group(3),'delta':float(m.group(5)),'kind':'troops','source':m.group(4),'raw':line}
    m=re.match(r'^T(\d+) ([AB]):([^\s]+) 備兵吸収 ([0-9.]+) source=(.*)',line)
    if m:
        return {'turn':int(m.group(1)),'side':_canonical_side(direction,m.group(2)),'officer':m.group(3),'delta':-float(m.group(4)),'kind':'reserve','source':m.group(5),'raw':line}
    m=re.match(r'^T(\d+) ([AB]):([^\s]+) .*? 備兵grant -> ([^\s]+) amount=([0-9.]+)',line)
    if m:
        return {'turn':int(m.group(1)),'side':_canonical_side(direction,m.group(2)),'officer':m.group(4),'delta':float(m.group(5)),'kind':'reserve','source':'備兵付与','raw':line}
    return None
...
def _turn_payload(turn, trace_turn, turn_logs, direction, ended_turn, final_state):
    ...
    if turn>ended_turn or not trace_turn:
        return {'turn':turn,'status':'battle_ended','startTroops':_pack_troops(start),'endTroops':_pack_troops(start),'turnStartChanges':[],'actions':[],'turnEndChanges':[]}
    ...
    pending=[]
    for line in body:
        change=_parse_change(line,direction)
        actor=_source_actor(line,order_names,direction)
        if change:
        ...
        if actor:
            ...
            if pending:
                bucket['changes'].extend(pending);pending=[]
            if not _is_technical(line):bucket['events'].append(_replace_sides(re.sub(r'^T\d+\s+','',line),direction))
        elif pending and (' -> ' in line or ' heal ' in line):
            unassigned.extend(pending);pending=[]
    unassigned.extend(pending)
    actions=[]
    for row in order:
        bucket=buckets.get((row['side'],row['officer']),{'events':[],'changes':[]})
        changes=_changes_with_before_after(bucket['changes'],current)
        events=[]
    ...
    return {'turn':turn,'status':'active','startTroops':_pack_troops(start),'endTroops':_pack_troops(end),'turnStartChanges':turn_start_changes,'actions':actions,'turnEndChanges':turn_end_changes}
...
def _build_example(candidate,target,direction,seed,outcome):
    ...
    final_state={}
    if ended:
        end_turn=_record(_record(trace.get('turns')).get(str(ended)))
        final_state=_state_map(end_turn.get('end_state'),direction)
    turns=[]
    for turn in range(1,9):
        trace_turn=_record(_record(trace.get('turns')).get(str(turn)))
        turns.append(_turn_payload(turn,trace_turn,logs_by_turn[turn],direction,ended,final_state))
        if trace_turn:
            final_state=_state_map(trace_turn.get('end_state'),direction) or final_state
    ...
    for direction in ('forward','reverse'):
        for block in _array(blocks.get(direction)):
            for rep in _array(_record(block).get('representative_traces')):
                ...
                if isinstance(seed,(int,float)) and winner in ('A','B','draw'):
                    rows.append({'direction':direction,'seed':int(seed),'outcome':_candidate_outcome(direction,winner)})
    return rows
...
def _scan_ranges(summary):
    ...
    for direction in ('forward','reverse'):
        for block in _array(sim.get(direction)):
            ...
            if isinstance(seed,(int,float)) and isinstance(trials,(int,float)):
                ranges.append((direction,int(seed),int(trials)))
    return ranges
...
def build_battle_examples(request_json, summary_json):
    ...
    candidate=_make(req['candidate']);target=_make(req.get('target_spec'))
    rate=float(summary.get('win_rate') or 0.0)
    targets={'win':1 if rate>=0.999999 else (0 if rate<=0.000001 else 2),'loss':1 if rate<=0.000001 else (0 if rate>=0.999999 else 2)}
    selected={'win':[],'loss':[]};seen=set()
    def add(direction,seed,outcome):
        key=(direction,int(seed))
        if outcome in selected and len(selected[outcome])<targets[outcome] and key not in seen:
            selected[outcome].append((direction,int(seed)));seen.add(key)
    ...
    if any(len(selected[k])<targets[k] for k in selected):
        for direction,start,count in _scan_ranges(summary):
            for seed in range(start,start+count):
                ...
                outcome=_outcome(candidate,target,direction,seed);add(direction,seed,outcome)
